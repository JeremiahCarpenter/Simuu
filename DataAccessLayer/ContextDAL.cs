...
namespace DataAccessLayer
{
    public class ContextDAL : IDisposable
    {
        ...
        public UserDAL User_FindByUserName(string userName)
        {
            UserDAL ProposedReturnValue = null;
            try
            {
                EnsureConnected();
                using (SqlCommand command = new SqlCommand("User_FindByUserName", _connection))
                {
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@UserName", userName);
                    using (SqlDataReader reader = command.ExecuteReader())
        ...
        public int Simuus_ObtainCountRelatedToUserID(int userID)
        {
            ...
            {
                EnsureConnected();
                using (SqlCommand command = new SqlCommand("Simuus_ObtainCountRelatedToUserID", _connection))
                {
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@UserID", userID);
                    object answer = command.ExecuteScalar();
